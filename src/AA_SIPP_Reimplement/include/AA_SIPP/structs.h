...
{
    // Node(생성자)
    // Node(매개변수 전달: int _i=-1, int _j=-1, double _g=-1, double _F=-1)
    // Node(초기화 목록 사용: i(_i),j(_j),g(_g),F(_F),Parent(nullptr){})
    Node(int _i=-1, int _j=-1, double _g=-1, double _F=-1):i(_i),j(_j),g(_g),F(_F),Parent(nullptr){}
    ~Node(){ Parent = nullptr; }
    int     i, j;           // 노드의 위치(좌표)
    double  size;           // 크기 정보(생성자에서 초기화하지 않음)
    double  g;              // 현재까지 이동한 거리(cost to come)
    double  F;              // 평가 함수 값(일반적으로 F = g + h, A*에서)
    double  heading;        // 각도값(생성자에서 초기화하지 않음)
    Node*   Parent;         // 경로 추적을 위한 이전 노드를 가리키는 포인터
    SafeInterval interval;  // 이 노드에서 가능한 시간 간격
};
