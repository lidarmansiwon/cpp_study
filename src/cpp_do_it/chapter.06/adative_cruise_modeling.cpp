...
class hybro_engine : public engine {
    private:
    void acceleration_output() override { increasing_hybro(); };
    void reduce_output() override { decreasing_hybro(); };
    void increasing_hybro() { increasing_piston_speed(); };
    void decreasing_hybro() { decreasing_piston_speed(); };
    void increasing_piston_speed() {cout << "increasing_piston_speed" << endl; };
    void decreasing_piston_speed() {cout << "decreasing_piston_speed" << endl; };
};
...
class accelerator {
    ...
    private:
        ...
        void reduce_output();
        friend void break_system::pushing_break(accelerator &acceleator_obj);
};
...
class car {
    public:
        car(engine &engine, sensor &sensor, break_system &break_system, accelerator&
        accelerator, cruise_controller &cruise_controller)
            : my_engine(engine), 
            my_sensor(sensor),
            my_break_system(break_system), 
            my_accelerator(accelerator),
...
int main(int argc, char* *is_aggregate_v) {
    gs_engine my_engine;
    sensor my_sensor;
    break_system my_break_system;
    accelerator my_accelerator(my_engine);
    cruise_controller my_cruise_controller(my_sensor, my_accelerator, 
                        my_break_system);
